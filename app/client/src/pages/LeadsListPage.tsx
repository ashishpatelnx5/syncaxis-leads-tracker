...
function describeSpecialFilter(params: URLSearchParams): string | null {
  ...
  if (params.get('agingBucket')) return `Open, aged ${params.get('agingBucket')}`;
  if (params.get('customerId')) return 'Leads for this customer';
  return null;
}
...
export function LeadsListPage() {
  ...
  const load = useCallback(() => {
    ...
    fetchLeads({
      ...
      cardCollected, inquirySource, productInterest, overdue, followUpDueDays, hasValue, hasErpRef, agingBucket, customerId,
      sortBy, sortDir,
      page, pageSize,
    })
      .then((res) => {
  ...
  }, [q, status, priority, leadType, assignedTo, leadGeneratedBy, cardCollected, inquirySource, productInterest, overdue, followUpDueDays, hasValue, hasErpRef, agingBucket, customerId, sortBy, sortDir, page, pageSize]);
  ...
  return (
    <div className="page">
      ...
      <div className="trend-period-toggle">
        <button type="button" className={`trend-period-btn${view === 'table' ? ' active' : ''}`} onClick={() => setView('table')}>Table</button>
        <button type="button" className={`trend-period-btn${view === 'board' ? ' active' : ''}`} onClick={() => setView('board')}>Board</button>
      </div>

      {view === 'board' && <LeadsBoard />}
        ...
        <table>
          <thead>
            <tr>
              ...
              {sortableHeader('Assigned To', 'InquiryAssignedTo', (
                <HeaderFilterDropdown
          ...
          <tbody>
            ...
            {!loading && items.length === 0 && (
              <tr><td colSpan={11} className="empty-state">No leads found.</td></tr>
            )}
